...
/*   Updated: 2023/11/08 23:22:50 by bgrhnzcn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_image(t_data *data)
{
	...
	while (i < data->tri_count)
	{
		...
		tris[i].p1.x += 1.; tris[i].p1.y += 1.;
		tris[i].p2.x += 1.; tris[i].p2.y += 1.;
		tris[i].p3.x += 1.; tris[i].p3.y += 1.;

		tris[i].p1.x *= 0.5 * WIDTH; tris[i].p1.y *= 0.5 * HEIGHT;
		tris[i].p2.x *= 0.5 * WIDTH; tris[i].p2.y *= 0.5 * HEIGHT;
		tris[i].p3.x *= 0.5 * WIDTH; tris[i].p3.y *= 0.5 * HEIGHT;
